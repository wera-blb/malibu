...
function renderSigel(sigelArray) {
  ...
  for (var i = 0; i < sigelArray.length; i++) {
    if (sigelArray[i].startsWith('ZDB-')) {
      // render Sigel starting with ZDB- as links
      sigelMitZDBAlsLinks +=
        '<a href="https://sigel.staatsbibliothek-berlin.de/nc/suche/?isil=' +
        sigelArray[i] +
        '" ' +
        'target="_blank">' +
        sigelArray[i] +
      ...
      // render other Sigel normally
      sigelMitZDBAlsLinks += sigelArray[i];
    }
    if (i + 1 < sigelArray.length) {
      //append separator if it is not the last item
...
function addBenennung(index, element) {
  ...
  $.getJSON(rvkApi, function (json) {
    if ('node' in json) {
      if (json.node.ancestor) {
        //Benennung des Knoten + Benennung des Vorfahrens
        $('.' + className).attr('title', json.node.benennung + ' <-- ' + json.node.ancestor.node.benennung);
      } else {
        //Benennung des Knoten
...
function renderDDC(ddcArray) {
  if (typeof ddcArray === 'string') {
    ...
    for (var i = 0; i < ddcArray.length; i++) {
      ...
      var ddcUrl =
        'https://coli-conc.gbv.de/cocoda/app/?fromScheme=http%3A%2F%2Fdewey.info%2Fscheme%2Fedition%2Fe23%2F&toScheme=http%3A%2F%2Furi.gbv.de%2Fterminology%2Frvk%2F&from=http%3A%2F%2Fdewey.info%2Fclass%2F' +
        ddc +
        '%2Fe23%2F';
...
function renderBestandSWB(bestandArray, id) {
  var bibArray = $.map(bestandArray, function (sigel) {
    if (sigel === '31') {
      return '<span style="border:2px solid red">31</span>';
    } else {
      return sigel;
    }
  });
  var outputString =
    'Insgesamt ' +
    bibArray.length +
    " Bibliotheken im <a href='https://swb.bsz-bw.de/DB=2.1/PPNSET?PPN=" +
    id +
    "&INDEXSET=21' target='_blank'>SWB</a> mit Bestand: " +
    bibArray.join(', ');
  return outputString;
}
...
function coins(currentRecord) {
  ...
  var coins =
    '<span class="Z3988" title="url_ver=Z39.88-2004&amp;ctx_ver=Z39.88-2004&amp;rft_val_fmt=info%3Aofi%2Ffmt%3Akev%3Amtx%3Abook';
